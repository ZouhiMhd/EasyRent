...
const TableVehicle = () => {
  ...
  const [selectedAntenna, setSelectedAntenna] = useState(1); // Ajouter un état pour l'antenne sélectionnée
  const [data, setData] = useState(null);
 ...
 const handleFilterChange = (event) => {
  ...
  const handleAntennaSelect = (antennaId) => {
    ...
    fetchVehicles(antennaId); // Appeler fetchVehicles avec l'antenne sélectionnée
  };
  ...
  const fetchVehicles = async (antennaId) => {
    try {
      const endpoint = `postes/${antennaId}/vehicules`;
      const res = await axios.get(`${API_BASE_URL}/${endpoint}`);
      setData(res.data);
